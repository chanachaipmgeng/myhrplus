/**
 * Generic Screen Definition Models
 * Models สำหรับเก็บข้อมูลหน้าจอ JSP จากระบบเก่าเพื่อใช้ในการ migrate
 * ใช้ได้กับทุก module (Company, Personal, Payroll, Time, Training, Welfare, Recruit, Appraisal, Settings)
 */
...
export interface JspFile {
  /** ชื่อไฟล์ JSP (เช่น CS008.jsp, PRU339.jsp) */
  filename: string;
  /** ประเภทไฟล์ */
  type: JspFileType;
 ...
 * ข้อมูลหน้าจอจากระบบเก่า (ใช้ได้กับทุก module)
 */
export interface ScreenDefinition {
  /** Menu Code (เช่น CO01A0102, PS01A0201) */
  menuCode: string;
  /** Menu Name (Thai) */
  menuNameThai: string;
  /** Menu Name (English) */
  ...
  basePath?: string;
  /** Route Path - route path สำหรับ Angular */
  routePath?: string;
}
...
export interface ScreenCategory {
  /** Category Code (เช่น CO01A, PS01A01) */
  categoryCode: string;
  /** Category Name (Thai) */
...
/**
 * Module Inventory
 * ข้อมูลทั้งหมดของ Module
 */
export interface ModuleInventory {
  /** Module Code (CO, PS, PR, TA, TR, WE, RE, AS, ST) */
  moduleCode: string;
  /** Module Name */
  moduleName: string;
  /** Base Path */
...
/**
 * Helper function: Create ScreenDefinition from inventory data
 * สร้าง ScreenDefinition จากข้อมูล inventory
 */
